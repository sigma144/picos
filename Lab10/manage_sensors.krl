ruleset manage_sensors {
    ...
    global {
        __testing = {
            ...
            "events": [
                ...
                {"domain": "gossip", "name": "new_sensor_state", "attrs":["state"]}
            ]
        }
    ...
    rule set_sensor_state {
        select when gossip new_sensor_state
        foreach subs:established("Tx_role", "temperature_sensor") setting (sub)
        event:send({
            "eci":sub{"Tx"},
            "domain":"emitter",
            "type":"new_state",
            "attrs": (event:attrs{"state"} == "paused" => {"pause":true} | {})
        });
    }
    ...
    rule install_gossip_temps {
        select when wrangler new_child_created
        installRuleset(event:attrs{"name"}, event:attrs{"eci"}, github_path+"Lab10/gossip_temps.krl")
    }
