ruleset gossip_temps {
    meta {
        ...
        shares __testing, getRumors, getSeen, getSeenRecord
    ...
    global {
        ...
        getSeenNumber = function(id) {
            nums = ent:rumors{id}.map(function(v) {v{"Message#"}})
            max = nums.reduce(function(x,n) {n > x => n | x}, 0).klog("Max")
            0.range(max).reduce(function(n,x) {
                n == x+1 && nums >< n => n | x
            }, -1)
        }
    ...
    rule collect_temperatures {
        ...
        always{
            ent:rumors{[ent:id, ent:message_num.as("String")]} := createRumor(temperature, time)
